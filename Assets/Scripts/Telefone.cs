...
public class Telefone : MonoBehaviour
{
    ...
    [FoldoutGroup("Phone Book"), ListDrawerSettings(NumberOfItemsPerPage = 3)]
    public List<PhoneNmbr> phoneNumberList = new List<PhoneNmbr>();

    [FoldoutGroup("Dialling")]
    public DialTones dialTones;

    AudioSource audioSource;

    [FoldoutGroup("Dialling"), ReadOnly]
    public string dialedNumber = "";
    [FoldoutGroup("Dialling"), Range(.0f, 5f), Tooltip("Tempo até tentar chamar depois de se digitar um número correcto")]
    public float callDelay = 3f;
    [FoldoutGroup("Dialling"), Range(.0f, 10f), Tooltip("Tempo até ir abaixo depois de se carregar num digito")]
    ...
    [FoldoutGroup("RingTone")]
    [ButtonGroup("RingTone/0"), Button("Ring"), DisableInEditorMode]
    public void RingToneA() { Ring("A"); }
    [ButtonGroup("RingTone/0"), Button("Zelda"), DisableInEditorMode]
    public void RingToneB() { Ring("B"); }
    [ButtonGroup("RingTone/0"), Button("Birthday"), DisableInEditorMode]
    public void RingToneC() { Ring("C"); }
    [ButtonGroup("RingTone/0"), Button("Random"), DisableInEditorMode]
